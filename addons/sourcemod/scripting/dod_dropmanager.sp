...
*
* Description:
*   Allows player to drop healthkit, ammobox or TNT.
...
public Plugin:myinfo =
{
	...
	description = "Allows player to drop healthkit, ammo box or TNT using 'dropammo' command",
...
public OnPluginStart()
{
	...
	menumode           = CreateConVar("dod_dropmanager_menumode",     "1",  "Whether or not show panel on drop\nIt lets player to choose items for dropping from a menu", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	ammosize           = CreateConVar("dod_dropammobox_clipsize",     "2",  "Determines number of clips a dropped ammobox contains", FCVAR_PLUGIN, true, 1.0, true, 5.0);
...
public OnMapStart()
{
	...
	// Also precache model of allied ammo box
	PrecacheModel(AlliesAmmoModel, true);

	// And dont forget about axis' one
	PrecacheModel(AxisAmmoModel,   true);
	PrecacheSound(AmmoSound,       true);

	// Model precache is needed to prevent client crash
	PrecacheModel(TNTModel,        true);

	// Sound doesn't crash client, but you still need to precache this to hear
...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetClientHealth(client) < 1)
	{
		...
		switch (GetConVarInt(deaddrop))
		{
			...
			case 4:
			{
				// Value 4 means random item. So get random item number then
				switch (GetRandomInt(0, 2))
				{
					// And check avaliability once again
					case 0: if (HasHealthkit[client]) CreateItem(client, Healthkit);
					case 1: if (HasAmmoBox[client])   CreateItem(client, Ammobox);
					case 2: if (HasTNT[client])       CreateItem(client, Bomb);
				}
			}
		}
	}
...
{
	// Only alive players can use this command
	if (IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		if (GetConVarBool(allowhealthkit) || GetConVarBool(allowammobox) || GetConVarBool(allowtnt))
		{
			...
			// It's needed to check for TNT avaliability here
			if (GetConVarBool(allowtnt)
			&& GetPlayerWeaponSlot(client, Slot_Bomb) != -1)
			{ HasTNT[client] = true; }

			// Check current ammo to allow or disallow dropping
			if (GetConVarBool(allowammobox)) PerformAmmunition(client, mode:check);
...
public DropMenuHandler(Handle:menu, MenuAction:action, client, param)
{
	...
	if (action == MenuAction_Select)
	{
		...
		switch (param)
		{
			// Since panel title is 0, indexes started by 1
...
CreateItem(client, index)
{
	...
	// Let's disable item/item collision. It can prevent huge lags
	SetEntProp(item, Prop_Send, "m_CollisionGroup", 4);

	// And now create an item depends on index
	switch (index)
	{
		// Now we can spawn an entity (item) depends on index
