...
LoadConVars()
{
	...
	AddConVar(AllowNade,            ValueType_Bool,  CreateConVar("dod_dropmanager_grenade",      "1",   "Whether or not allow grenades dropping\nPlayers may equip any grenades without limits and restrictions",                        FCVAR_PLUGIN, true, 0.0,   true, 1.0));
...
#if defined REALISM
	AddConVar(RagdollStay,          ValueType_Bool,  CreateConVar("dod_dropmanager_ragdoll_stay", "0",   "Whether or not dont remove player ragdolls (corpses) from the groud during the round\nNOTE: If server will lag - disable this", FCVAR_PLUGIN, true, 0.0,   true, 1.0));
#endif
...
#if defined REALISM
	AddConVar(AmmoBox_ClipLimit,    ValueType_Bool,  CreateConVar("dod_dropammobox_cliplimit",    "0",   "Whether or not limit amount of clips that player may have\nAdapted for Realism matches",                                        FCVAR_PLUGIN, true, 0.0,   true, 1.0));
#endif
